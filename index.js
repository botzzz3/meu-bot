	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `OPA @${num.split('@')[0]}\nBEM VINDO DIGITE !HELP2*${mdata.subject}*`
	    ...
	    case 'kapankah':
            ...
            break
				...
				/*if (isDaftar) return reply(mess.only.daftarB)*/
					...
					break
       Case'bom dia '
tujuh = fs.readFileSync('./assets/bom dia.mp3');
   break                    client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                                        /*if (!isDaftar) return reply(mess.only.daftarB)*/
						reply(mess.wait)
